...
const empty: Omit<Dhikr, "id"> = { title: "", text: "", count: 1, category: "أذكار الصباح" };
...
function AdminAzkar() {
  ...
  const openEdit = (d: Dhikr) => {
    setEditing(d);
    setForm({ title: d.title, text: d.text, count: d.count, category: d.category });
  };
  const close = () => {
    setEditing(null);
    setCreating(false);
  ...
  const save = () => {
    ...
    if (editing) updateDhikr(editing.id, form);
    else addDhikr(form);
    close();
  };
          ...
          <li key={d.id} className="rounded-2xl bg-card border border-border p-3 shadow-card">
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex flex-col gap-1.5 shrink-0">
                ...
                <button
                  onClick={() => confirm("حذف هذا الذكر؟") && deleteDhikr(d.id)}
        ...
        <div className="fixed inset-0 z-50 bg-black/40 grid place-items-end sm:place-items-center p-3" onClick={close}>
          <div className="w-full max-w-md bg-card rounded-3xl p-5 shadow-soft max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <button onClick={save} className="w-full inline-flex items-center justify-center gap-2 rounded-xl gradient-primary text-primary-foreground py-2.5 font-bold text-sm">
